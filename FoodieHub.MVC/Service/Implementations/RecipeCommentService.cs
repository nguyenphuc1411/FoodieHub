...
namespace FoodieHub.MVC.Service.Implementations
{
    public class RecipeCommentService : IRecipeCommentService
    {
        ...
        public RecipeCommentService(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<APIResponse<List<GetComment>>> GetCommentsOfRecipeAsync(int recipeID, string order = "asc")
        {
            var response = await _httpClient.GetAsync($"comments/recipe/{recipeID}?order={order}");
        ...
        public async Task<APIResponse> CreateRecipeCommentAsync(CreateRecipeComment comment)
        {
            var response = await _httpClient.PostAsJsonAsync("comments/recipe", comment);
            return await response.Content.ReadFromJsonAsync<APIResponse>();
        }
    }
}
