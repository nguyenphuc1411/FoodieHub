...
namespace FoodieHub.API.Services.Implementations
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<ServiceResponse> Get(int? pageSize, int? currentPage, DateOnly? orderDate, string? orderKey, bool? isDesc, string? status)
        {
            ...
            {
                listOrders = listOrders.Where(x => DateOnly.FromDateTime(x.OrderedAt) == orderDate.Value);
            }
            if (!string.IsNullOrEmpty(status))
            {
                listOrders = listOrders.Where(x => x.Status.ToLower().Contains(status.ToLower()));
            }

            if (!string.IsNullOrEmpty(orderKey) && isDesc.HasValue)
            {
                if (orderKey == "PRICE")
                {
                    if (isDesc.Value)
                    {
                        listOrders = listOrders.OrderByDescending(x => x.TotalAmount);
                    }
                    else
                    {
                        listOrders = listOrders.OrderBy(x => x.TotalAmount);
                    }
                }
                if (orderKey == "ORDEREDDATE")
                {
                    if (isDesc.Value)
                    {
                        listOrders = listOrders.OrderByDescending(x => x.OrderedAt);
                    }
                    else
                    {
                        listOrders = listOrders.OrderBy(x => x.OrderedAt);
                    }
                }
            }
            ...
            if (pageSize.HasValue && currentPage.HasValue)
            {
                ...
                return new ServiceResponse
                {
                    ...
                    Data = listOrders.Paginate(pageSize.Value, currentPage.Value),
                    StatusCode = 200
                };
            }
            return new ServiceResponse
            {
                Success = true,
                Message = "Get orders successfully",
                Data = await listOrders.ToListAsync(),
                StatusCode = 200
            };
        }
        ...
        public async Task<ServiceResponse> GetByUser(int? pageSize, int? currentPage)
        {
            var userID = _authService.GetUserID();
        ...
        public async Task<ServiceResponse> GetDetail(int orderID)
        {
            var order = await _context.Orders.Where(x => x.OrderID == orderID)
